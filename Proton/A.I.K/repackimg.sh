...
case $plat in
  macos)
    cpio="env DYLD_LIBRARY_PATH="$bin/$arch" "$bin/$arch/x86_64/cpio"";
    statarg="-f %Su";
    dd() { DYLD_LIBRARY_PATH="$bin/$arch" "$bin/$arch/x86_64/dd" "$@"; }
    java() { "/Library/Internet Plug-Ins/JavaAppletPlugin.plugin/Contents/Home/bin/java" "$@"; }
    lzop() { DYLD_LIBRARY_PATH="$bin/$arch" "$bin/$arch/x86_64/lzop" "$@"; }
    xz() { DYLD_LIBRARY_PATH="$bin/$arch" "$bin/$arch/x86_64/xz" "$@"; }
  ;;
  linux)
    cpio=cpio;
    statarg="-c %U";
...
if [ "$original" ]; then
  ...
  case $ramdiskcomp in
    ...
    bzip2) compext=bz2;;
    lz4) repackcmd="$bin/$arch/x86_64/lz4 $level -l";;
    *) abort; exit 1;;
  esac;
  cd ramdisk;
...
if [ "$imgtype" != "KRNL" ]; then
  kernel=`ls *-Image`;                 echo "kernel = $kernel";
...
if [ -f split_img/*-mtktype ]; then
  ...
  "$bin/$arch/x86_64/mkmtkhdr" --kernel "$kernel" --$mtktype "$ramdisk" >/dev/null;
  if [ ! $? -eq "0" ]; then
    abort;
    exit 1;
  fi;
...
echo " ";
case $imgtype in
  AOSP) "$bin/$arch/x86_64/mkbootimg" --kernel "$kernel" --ramdisk "$ramdisk" "${second[@]}" --cmdline "$cmdline" --board "$board" --base $base --pagesize $pagesize --kernel_offset $kerneloff --ramdisk_offset $ramdiskoff --second_offset "$secondoff" --tags_offset "$tagsoff" --os_version "$osver" --os_patch_level "$oslvl" $hash "${dtb[@]}" -o $outname;;
  AOSP-PXA) "$bin/$arch/x86_64/pxa-mkbootimg" --kernel "$kernel" --ramdisk "$ramdisk" "${second[@]}" --cmdline "$cmdline" --board "$board" --base $base --pagesize $pagesize --kernel_offset $kerneloff --ramdisk_offset $ramdiskoff --second_offset "$secondoff" --tags_offset "$tagsoff" --unknown $unknown "${dtb[@]}" -o $outname;;
  ELF) "$bin/$arch/x86_64/elftool" pack -o $outname header="$header" "$kernel" "$ramdisk",ramdisk "${rpm[@]}" "$cmdname"@cmdline >/dev/null;;
  KRNL) "$bin/$arch/x86_64/rkcrc" -k "$ramdisk" $outname;;
  U-Boot)
    test "$type" == "Multi" && uramdisk=(:"$ramdisk");
    "$bin/$arch/x86_64/mkimage" -A $uarch -O $os -T $type -C $comp -a $addr -e $ep -n "$name" -d "$kernel""${uramdisk[@]}" $outname >/dev/null;
  ;;
...
if [ -f split_img/*-sigtype ]; then
  ...
  case $sigtype in
    ...
    BLOB)
      ...
      "$bin/$arch/x86_64/blobpack" tempblob $blobtype unsigned-new.img >/dev/null;
      cat tempblob >> image-new.img;
      rm -rf tempblob;
    ;;
    CHROMEOS) "$bin/$arch/x86_64/futility" vbutil_kernel --pack image-new.img --keyblock $rel/chromeos/kernel.keyblock --signprivate $rel/chromeos/kernel_data_key.vbprivk --version 1 --vmlinuz unsigned-new.img --bootloader $rel/chromeos/empty --config $rel/chromeos/empty --arch arm --flags 0x1;;
    DHTB)
      "$bin/$arch/x86_64/dhtbsign" -i unsigned-new.img -o image-new.img >/dev/null;
...
if [ -f split_img/*-lokitype ]; then
  ...
  if [ -f aboot.img ]; then
    "$bin/$arch/x86_64/loki_tool" patch $lokitype aboot.img unlokied-new.img image-new.img >/dev/null;
    if [ ! $? -eq "0" ]; then
