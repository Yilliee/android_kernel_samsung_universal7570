...
case $plat in
  macos)
    ...
    file() { DYLD_LIBRARY_PATH="$bin/$arch" "$bin/$arch/x86_64/file" "$@"; }
    lzma() { DYLD_LIBRARY_PATH="$bin/$arch" "$bin/$arch/x86_64/xz" "$@"; }
    lzop() { DYLD_LIBRARY_PATH="$bin/$arch" "$bin/$arch/x86_64/lzop" "$@"; }
    tail() { DYLD_LIBRARY_PATH="$bin/$arch" "$bin/$arch/x86_64/tail" "$@"; }
...
if [ "$(echo $imgtest | awk '{ print $2 }' | cut -d, -f1)" = "signing" ]; then
  ...
  case $sigtype in
    BLOB)
      cd split_img;
      cp -f "$img" "$file";
      "$bin/$arch/x86_64/blobunpack" "$file" | tail -n+5 | cut -d" " -f2 | dd bs=1 count=3 > "$file-blobtype" 2>/dev/null;
      mv -f "$file."* "$file";
      cd ..;
    ;;
    CHROMEOS) "$bin/$arch/x86_64/futility" vbutil_kernel --get-vmlinuz "$img" --vmlinuz-out "split_img/$file";;
    ...
    ;;
    SIN)
      "$bin/$arch/x86_64/kernel_dump" split_img "$img" >/dev/null;
...
if [ "$(echo $imgtest | awk '{ print $3 }')" = "LOKI" ]; then
  ...
  echo " ";
  "$bin/$arch/x86_64/loki_tool" unlok "$img" "split_img/$file" >/dev/null;
  img="$file";
fi;
...
cd split_img;
case $imgtype in
  AOSP) "$bin/$arch/x86_64/unpackbootimg" -i "$img";;
  AOSP-PXA) "$bin/$arch/x86_64/pxa-unpackbootimg" -i "$img";;
  ELF)
    mkdir elftool_out;
    "$bin/$arch/x86_64/elftool" unpack -i "$img" -o elftool_out >/dev/null;
    mv -f elftool_out/header "$file-header" 2>/dev/null;
    rm -rf elftool_out;
    "$bin/$arch/x86_64/unpackelf" -i "$img";
  ;;
  KRNL) dd bs=4096 skip=8 iflag=skip_bytes conv=notrunc if="$img" of="$file-ramdisk.cpio.gz" 2>&1 | tail -n+3 | cut -d" " -f1-2;;
  U-Boot)
    "$bin/$arch/x86_64/dumpimage" -l "$img";
    "$bin/$arch/x86_64/dumpimage" -l "$img" > "$file-header";
    ...
    rm -rf "$file-header";
    "$bin/$arch/x86_64/dumpimage" -i "$img" -p 0 "$file-Image";
    if [ ! $? -eq "0" ]; then
      cleanup;
      abort;
      exit 1;
    ...
    if [ "$(cat "$file-type")" = "Multi" ]; then
      "$bin/$arch/x86_64/dumpimage" -i "$img" -p 1 "$file-ramdisk.cpio.gz";
    else
...
if [ "$(file -m ../$rel/androidbootimg.magic *-Image | cut -d: -f2 | awk '{ print $1 }')" = "MTK" ]; then
  mtk=1;
  echo " ";
  echo "MTK header found in Image, removing...";
  dd bs=512 skip=1 conv=notrunc if="$file-Image" of=tempzimg 2>/dev/null;
  mv -f tempzimg "$file-Image";
fi;
...
if [ "$(echo $mtktest | awk '{ print $1 }')" = "MTK" ]; then
  if [ ! "$mtk" ]; then
    echo " ";
    echo "Warning: No MTK header found in Image!";
    mtk=1;
  fi;
...
if [ -f *-dtb ]; then
  ...
  if [ "$imgtype" = "ELF" ]; then
    case $dtbtest in
      QCDT|ELF) ;;
      *) echo " ";
         echo "Non-QC DTB found, packing Image and appending...";
         gzip --no-name -9 "$file-Image";
         mv -f "$file-Image.gz" "$file-Image";
         cat "$file-dtb" >> "$file-Image";
         rm -f "$file-dtb"*;;
    esac;
  fi;
fi;
...
case $ramdiskcomp in
  ...
  lzma) ;;
  bzip2) compext=bz2;;
  lz4) unpackcmd="$bin/$arch/x86_64/lz4 -dcq";;
  empty) compext=empty;;
  *) compext="";;
esac;
if [ "$compext" ]; then
